...
export default async function processWeaponCommand(
  input: string
): Promise<Weapon[]> {
  if (input) {
    var weaponCommand = new WeaponCommand(input);
    const results = await getInventoryItemByName(input);
    weaponCommand.setWeaponResults(results);
    for (let weapon of weaponCommand.weaponResults) {
      let powerCapValues = await processPowerCapHashes(
        weapon.rawData.powerCapHashes
      );
      weapon.setPowerCapValues(powerCapValues);
      let [sockets, intrinsic] = await processSocketData(
        weapon.rawData.socketData
      );
      let baseArchetype = await processBaseArchetype(weapon.rawData, intrinsic);
...
async function processPowerCapHashes(
  powerCapHashes: number[]
): Promise<number[]> {
  if (powerCapHashes) return await getPowerCap(powerCapHashes);
  else throw Error("No Power Cap hashes found");
}

async function processBaseArchetype(
...
async function processSocketData(
  ...
  for (let category of socketData.socketCategories) {
    if (category.socketCategoryHash == SocketCategoryHash.INTRINSICS) {
      ...
      intrinsic = await processSocketIntrinisic(socket);
    }
    if (category.socketCategoryHash == SocketCategoryHash.WEAPON_PERKS) {
      sockets = await processSocketPerks(
...
async function processSocketIntrinisic(
  ...
  let plugItemHash = await getPlugItemHash(socketEntry.reusablePlugSetHash);

  let item = await getInventoryItemByHash(plugItemHash);
...
async function processSocketPerks(
  ...
  for (let index of socketIndices) {
    ...
    let plugCategoryHash = await getSocketTypeHash(socketTypeHash);
    ...
    var plugItems = await getPlugItems(plugSetHash);

    let perks = [];
    let items = await getInventoryItemsByHash(
