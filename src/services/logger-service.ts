...
function logToTransport(logObject: ILogObject) {
  ...
  let message =
    `${logObject.date.toISOString()} ${logObject.logLevel.toUpperCase()} ` +
    `[${logObject.loggerName} ${logObject.filePath}:${logObject.lineNumber}${
      logObject.functionName !== undefined ? " " + logObject.functionName : ""
    }]\n`;
  for (const argument of logObject.argumentsArray) {
    if (Object.prototype.hasOwnProperty.call(argument, "isError")) {
      ...
      message +=
        "\nL" +
        argumentParsed?.codeFrame?.lineNumber +
        " " +
        argumentParsed.codeFrame?.relevantLine.trimStart();
    } else {
      message += argument;
    }
  }
